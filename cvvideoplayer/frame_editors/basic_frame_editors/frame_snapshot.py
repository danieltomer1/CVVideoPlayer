...
class FrameSnapshot(BaseFrameEditCallback):
    ...
    def __init__(
        self, output_video_path: Path, output_shape: Tuple[int, int], frame_num: int
    ):
    ...
    def before_frame_resize(
        self, video_player: VideoPlayer, frame: np.ndarray, frame_num: int
    ):
        if frame_num == self._frame_num:
            cv2.imwrite(
                str(self._output_video_path), cv2.resize(frame, self._output_shape)
            )
