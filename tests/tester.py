...
def create_video_player_for_snapshot(
    ...
    output_path,
    frame_num: int,
    frame_edit_callbacks: List[BaseFrameEditCallback],
):
...
def run_test(
    key_recordings: List[keyboard.Key],
    frame_num,
    record: bool,
    test_name: str,
    ...
    if (
        frame_edit_callbacks is None
    ):  # refer to https://pylint.readthedocs.io/en/latest/user_guide/messages/warning/dangerous-default-value.html
